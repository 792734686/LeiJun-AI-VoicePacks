function [KTP,EF] = cr_elementKF_iTR3(ENC,ENC0,Eepi,we,wk,wg,t,Rxy)
%ENC ----- 节点坐标，全局，行
%NUxy ----- 结点平动位移，全局，列
...
Thetaxy1 = Array_by_Quaternions(Rd1);
Thetaxy2 = Array_by_Quaternions(Rd2);
Thetaxy3 = Array_by_Quaternions(Rd3);
Thetaxy = [Thetaxy1,Thetaxy2,Thetaxy3];
% -----------------------------------
...
% Nuxy 单元结点平动位移，局部，列
Npxy = [Nuxy;Thetaxy];
ke = estiffmLC_iTR3(ENC0,we,wk,wg,t);
...
KGM = Te'*P'*M*P*Te;
f_in = Te'*P'*H'*f;
ef = eforcemLC_iTR3(ENC0,Eepi,we,wk,wg,t);
ef = Te0'*ef;
EF = ef-f_in;
% 载荷切线刚度阵KL
n1e = ef(1:3);
...
n3e = ef(13:15);
m3e = ef(16:18);
Fp = [skew(n1e);skew(m1e);skew(n2e);skew(m2e);skew(n3e);skew(m3e)];
KL = Te'*Fp*G*Te;
KT = KM+KGR+KGP+KGM;
% KT = 0.5*(KT+KT');
% 对称化，考虑随动载荷
