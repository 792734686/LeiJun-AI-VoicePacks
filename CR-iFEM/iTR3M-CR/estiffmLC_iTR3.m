 function EK = estiffmLC_iTR3( ENC,we,wk,wg,t )
...
Bb1 = B1(4:6,:);
Bs1 = B1(7:8,:);
EK1 = (we*(Bm1'*Bm1)+wk*t^2*(Bb1'*Bb1)+wg*2/3*(Bs1'*Bs1))*det(JCB)*-27/96;
Bm2 = B2(1:3,:);
Bb2 = B2(4:6,:);
Bs2 = B2(7:8,:);
EK2 = (we*(Bm2'*Bm2)+wk*t^2*(Bb2'*Bb2)+wg*2/3*(Bs2'*Bs2))*det(JCB)*25/96;
Bm3 = B3(1:3,:);
Bb3 = B3(4:6,:);
Bs3 = B3(7:8,:);
EK3 = (we*(Bm3'*Bm3)+wk*t^2*(Bb3'*Bb3)+wg*2/3*(Bs3'*Bs3))*det(JCB)*25/96;
Bm4 = B4(1:3,:);
Bb4 = B4(4:6,:);
Bs4 = B4(7:8,:);
EK4 = (we*(Bm4'*Bm4)+wk*t^2*(Bb4'*Bb4)+wg*2/3*(Bs4'*Bs4))*det(JCB)*25/96;
EK = EK1+EK2+EK3+EK4;
% -----------------------------------------------------------------
% EK = Te'*EK*Te;
% EK = t*EK;
